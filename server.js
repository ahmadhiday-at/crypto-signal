...
// --- API: Get Market Analysis (MTF Engine) ---
app.get('/api/market/:market', async (req, res) => {
    try {
        ...
        if (!marketSnapshot) {
            ...
            marketSnapshot = await updateMarketSnapshot(market);
        }
        if (!marketSnapshot) {
...
app.get('/api/config', (req, res) => {
    const marketList = process.env.MARKET_LIST ? process.env.MARKET_LIST.split(',') : [];
    const tfList = process.env.TIMEFRAME_LIST ? process.env.TIMEFRAME_LIST.split(',') : [];
    const tfLabels = process.env.TIMEFRAME_LABELS ? process.env.TIMEFRAME_LABELS.split(',') : [];
    res.json({
        markets: marketList.map(code => ({ code, label: code })),
        timeframes: tfList.map((code, i) => ({ code, label: tfLabels[i] || code }))
    });
});

// --- API: Get Snapshot ---
app.get('/api/:market/:timeframe', async (req, res) => {
    try {
...
app.get('/api/reset', async (req, res) => {
    try {
        ...
        const dirsToClear = [path.join(__dirname, 'data/chart'), path.join(__dirname, 'data/market')];
        for (const dir of dirsToClear) {
            try {
                const files = await fs.promises.readdir(dir);
                for (const file of files) {
                    if (file.endsWith('.json')) await fs.promises.unlink(path.join(dir, file));
                }
...
// --- View: Render Market Dashboard ---
app.get('/view/market/:market', (req, res) => {
    try {
...
// --- View: Render Chart Page ---
app.get('/view/:market/:timeframe', (req, res) => {
    try {
...
wss.on('connection', (ws) => {
    ...
    ws.on('close', () => {
        try {
            if (ws.currentKey && clients.has(ws.currentKey)) clients.get(ws.currentKey).delete(ws);
        } catch (err) {
...
function scheduleCronJobs() {
    const markets = process.env.MARKET_LIST ? process.env.MARKET_LIST.split(',') : ['BTCUSDT'];
    const timeframes = process.env.TIMEFRAME_LIST ? process.env.TIMEFRAME_LIST.split(',') : ['15m'];
    const tfToCron = { '1m': '*/1 * * * *', '5m': '*/5 * * * *', '15m': '*/15 * * * *', '30m': '*/30 * * * *', '1h': '0 * * * *', '4h': '0 */4 * * *', '1d': '0 0 * * *', '1w': '0 0 * * 0' };
    markets.forEach(market => {
        timeframes.forEach(tf => {
            const cronExpr = tfToCron[tf];
            if (cronExpr) cron.schedule(cronExpr, async () => {
                console.log(`[Cron] Scheduled update for ${market}/${tf}`);
                await updateMarketData(market, tf);
            });
        });
    });
...
async function updateMarketSnapshot(market) {
    try {
        ...
        const roles = { htf: process.env.TF_HTF || '1d', structure: process.env.TF_STRUCTURE || '4h', mid: process.env.TF_MID || '1h', entry: process.env.TF_ENTRY || '15m' };
        const tfList = Object.values(roles);
        const tfToWeightKey = { '1d': 'WEIGHT_1D', '4h': 'WEIGHT_4H', '1h': 'WEIGHT_1H', '15m': 'WEIGHT_15M', '1w': 'WEIGHT_1W', '1m': 'WEIGHT_1M' };
        ...
        for (const tf of tfList) {
            ...
            const weightKey = tfToWeightKey[tf] || 'WEIGHT_DEFAULT';
            const weight = parseFloat(process.env[weightKey] || '1.0');
            if (data.support) {
...
async function initializeAllSnapshots() {
    ...
    const markets = process.env.MARKET_LIST ? process.env.MARKET_LIST.split(',') : ['BTCUSDT'];
    const timeframes = process.env.TIMEFRAME_LIST ? process.env.TIMEFRAME_LIST.split(',') : ['15m'];
    const roles = [process.env.TF_HTF, process.env.TF_STRUCTURE, process.env.TF_MID, process.env.TF_ENTRY].filter(Boolean);
        ...
        for (const timeframe of allRequiredTf) {
            try {
                ...
                if (!data) {
                    ...
                    let tfSeconds = 900; 
                    if (timeframe === '1w') tfSeconds = 604800;
                    ...
                    else if (timeframe === '1m') tfSeconds = 60;
...
async function updateMarketData(market, timeframe) {
    try {
        ...
        if (timeframe === (process.env.TF_ENTRY || '15m')) await updateMarketSnapshot(market);
    } catch (err) {
        console.error(`Critical Update Error [${market}/${timeframe}]:`, err);
    }
}

binance.connectWebSocket((market, price) => {
    try {
    ...
    await updateMarketData(market, timeframe);
}, process.env.MARKET_LIST ? process.env.MARKET_LIST.split(',') : ['BTCUSDT']);

server.listen(PORT, async () => {
    console.log(`Trading Chart Server running on http://localhost:${PORT}`);
    await initializeAllSnapshots();
    scheduleCronJobs();
});
